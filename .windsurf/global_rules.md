...
- Before noisy commands, prefer the Rust-native `wf-core run -- <command>`.
...
- If the command uses pipes, redirects, `&&`, `||`, or shell syntax, use `wf-core run --shell -- "<command>"`.
...
- Run noisy commands through `wf-core run --` before raw output is produced.
